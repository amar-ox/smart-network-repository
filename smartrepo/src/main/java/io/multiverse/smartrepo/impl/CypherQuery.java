...
public class CypherQuery {
	...
	public static class Graph {
		public static final String CREATE_HOST = "CREATE (h:Host:%s:%s {name: '%s', hostname: '%s', macAddress: '%s', platform: '%s', "
				+ "asn: '%s', bgpStatus: '%s', bgpId: '%s', hwsku: '%s'});";
		public static final String CREATE_LTP = "MATCH (r:Host) WHERE r.name = '%s' "
				+ "CREATE (r)-[:HAS_PORT]->(:Port {name: '%s', adminStatus: '%s', operStatus: '%s', "
				+ "index: '%s', speed: %s, mtu: %s});";
		public static final String CREATE_LOOPBACK = "MATCH (r:Host) WHERE r.name = '%s' "
				+ "CREATE (r)-[:HAS_PORT]->(p:Port:Loopback {name: '%s', adminStatus: '%s', operStatus: '%s', "
				+ "index: '%s', speed: %s, mtu: %s});";
		public static final String CREATE_BRIDGE = "MATCH (r:Host) WHERE r.name = '%s' "
				+ "CREATE (r)-[:HAS_PORT]->(p:Port:Bridge {name: '%s', adminStatus: '%s', operStatus: '%s', "
				+ "index: '%s', speed: %s, mtu: %s});";
		...
		public static final String CREATE_IP4CTP = "MATCH (:Host {name:'%s'})-[:HAS_PORT]->(l:Port {name:'%s'}) "
			//	+ "CREATE (l)-[:HAS_INTERFACE]->(ipc:IPv4Endpoint {ipAddr:'%s', netMask:'%s', netAddr: '%s'});";
				+ "SET l.ipAddr='%s', l.netMask='%s', l.netAddr='%s';";
		public static final String CREATE_SVICTP = "MATCH (:Host {name:'%s'})-[:HAS_PORT]->(l:Bridge {name:'%s'}) "
		...
		public static final String CREATE_IPCONN = "MATCH (:Host {name:'%s'})-[:HAS_PORT]->(:Port {name:'%s'})-[:HAS_INTERFACE]->(s:IPv4Endpoint) "
				...
				+ "WITH DISTINCT s,d "
				+ "CREATE (s)-[:HAS_IP_CONNECTION]->(d);";
		public static final String CREATE_BGP = "MATCH(r:Host {name:'%s'})-[:HAS_PORT]->(p:Port {ipAddr:'%s'})\r\n"
				+ "CREATE (p)-[:HAS_BGP_PEER]->(b:BgpPeer {peerIp:'%s', peerId:'%s', peerAs:'%s', "
		...
		public static final String CREATE_FIB_ROUTE_ITF = ""
				+ "MATCH (h:Host {name:'%s'})-[:HAS_PORT]->(rEx:Port {name:'%s'})\r\n"
		...
		public static final String CREATE_FIB_ROUTE_SVI = ""
				+ "MATCH (h:Host {name:'%s'})-[:HAS_PORT]->(:Bridge)-[:CONTAINS_VLAN]->(rEx:Vlan {svi:'%s'})\r\n"
				+ "CREATE (h)-[:HAS_FIB_ROUTE]->(r:FibRoute {to: '%s', via: '%s', type: '%s', cost: %s})-[:EGRESS_INTERFACE]->(rEx);";
		...
		public static final String AUTO_VLAN_MEMBER = "MATCH (h:Host)\r\n"
				+ "MATCH (h)-[:HAS_PORT]-(:Bridge)-[:CONTAINS_VLAN]->(svi:Vlan)\r\n"
				+ "MATCH (h)-[:HAS_PORT]->(l:Port) WHERE l.inVlan = svi.vlanID AND l.ipAddr IS NULL\r\n"
				+ "CREATE (l)-[b:IS_VLAN_MEMBER {mode: l.mode}]->(svi)\r\n"
				+ "RETURN h, svi, b;";
		public static final String AUTO_BGP_NEIGHBORS = "MATCH (b1:BgpPeer)<-[:HAS_BGP_PEER]-(c1:Port)-[:HAS_LINK]->(c2:Port)-[:HAS_BGP_PEER]->(b2:BgpPeer)\r\n"
				+ "WHERE b1.peerIp=c2.ipAddr AND b2.peerIp=c1.ipAddr AND b1.state = 'Established'\r\n"
				+ "WITH DISTINCT b1,b2\r\n"
	...
	public static class Constraints {
		...
		public static final String UNIQUE_IP4CTP = "CREATE CONSTRAINT unique_ip_address IF NOT EXISTS FOR (c:Port) REQUIRE (c.ipAddr) IS UNIQUE";
